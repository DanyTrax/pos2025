...
class ControladorVentas
{
	...
	{
		$tabla = "ventas";
		$respuesta = ModeloVentas::mdlFilterBy($tabla, $fechaInicial, $fechaFinal, $medioPago, $formaPago);
		return $respuesta;
	}
	...
	static public function ctrCrearVenta()
	{
		if (isset($_POST["nuevaVenta"])) {
			...
			$fecha = date('Y-m-d');
			$hora = date('H:i:s');
			$fecha_abono = $fecha . ' ' . $hora;

			/* VALIDACIÓN DEL CÓDGIGO DE VENTA A CREAR */
			$lastCodVenta = ModeloVentas::mdlLastCodVenta("ventas");
			$_POST["nuevaVenta"] = $lastCodVenta[0] + 1;

			if ($_POST["nuevoMetodoPago"] == "Completo") {
				$datos = array(
				...
				$datos = array(
					...
					"fecha_venta" => $fecha_abono,
					"id_vend_abono" => $_POST["idVendedor"],
					"abono" => $_POST["nuevoValorEfectivo"] ? str_replace(",", "", $_POST["nuevoValorEfectivo"]) : 0,
					"fecha_abono" => $fecha_abono,
					"pago" => $_POST["pago"],
					"medio_pago" => $_POST['nuevoMedioPago']
				);
			...
			if ($respuesta == "ok") {
				$_valor = $_POST['nuevoValorEfectivo'] ? str_replace(",", "", $_POST['nuevoValorEfectivo']) : str_replace(",", "", $_POST['totalVenta']);
				if ($_POST["nuevoMetodoPago"] == "Se Debe") {
					$_valor = 0;
				}
				$datos = [
					"id_vendedor" => $_POST["idVendedor"],
	...
	static public function ctrEditarVenta()
	{
		...
		if (isset($_POST["editarVenta"])) {
			...
			// 			   "impuesto"=>$_POST["nuevoPrecioImpuesto"],
			// 			   "nuevoMetodoPago"=>$_POST["nuevoPrecioNeto"],
			// 			   "total"=>$_POST["totalVenta"],
			// 			   "detalle"=>$_POST["detalle"],
			...
			if ($_POST["nuevoMetodoPago"] == "Completo") {
				...
				$datos = array(
					...
					"fecha_venta" => $fecha_abono,
					"id_vend_abono" => $_POST["idVendedor"],
					"abono" => str_replace(",", "", $_POST["nuevoValorEfectivo"]),
					"fecha_abono" => $fecha_abono,
					"pago" => $_POST["pago"],
					"medio_pago" => $_POST["nuevoMedioPago"],
				);
	...
	public function ctrDescargarReporte()
	{
		...
		if (isset($_GET["reporte"])) {
			...
			echo utf8_decode("<table border='0'> 
					...
					<td style='font-weight:bold; border:1px solid #eee;'>ESTADO DE PAGO</td>
					...
					<td style='font-weight:bold; border:1px solid #eee;'>FECHA VENTA</td>
					<td style='font-weight:bold; border:1px solid #eee;'>FECHA ABONO</td>
					</tr>");
			...
			foreach ($ventas as $row => $item) {
				...
				$empresa = ControladorUsuarios::ctrMostrarUsuarios("id", $item["empresa"]);
				...
				echo utf8_decode("</td>
					...
					<td style='border:1px solid #eee;'>" . substr($item["fecha_abono"], 0, 10) . "</td>		
		 			</tr>");
			}
	...
	static public function ctrSumaTotalVentas3()
	{

		$tabla3 = "ventas";

		$respuesta = ModeloVentas::mdlSumaTotalVentas3($tabla3);
	...
	static public function ctrSumaTotalVentas1()
	{

		$tabla1 = "usuarios";

		$respuesta = ModeloVentas::mdlSumaTotalVentas1($tabla1);
		//$respuesta['total'] = 500;


		return $respuesta;
	}
	static public function ctrSumaTotalVentas2()
	{

		$tabla1 = "usuarios";

		$respuesta = ModeloVentas::mdlSumaTotalVentas2($tabla1);
		//$respuesta['total'] = 500;
	...
	{
		if (isset($_POST["idUsuarioAbo"]) && isset($_POST["idVentaAbo"])) {
			if (preg_match('/^[0-9]+$/', $_POST["idUsuarioAbo"]) && preg_match('/^[0-9]+$/', $_POST["idVentaAbo"]) && preg_match('/^[0-9.,]+$/', $_POST["nuevoAbono"])) {
				$infoVenta = ModeloVentas::mdlMostrarVentas("ventas", "id", $_POST["idVentaAbo"]);
				$nuevoAbono = $infoVenta["abono"] + (str_replace(",", "", $_POST["nuevoAbono"]));
				$metodo = "Abono";
				$ulabono = (str_replace(",", "", $_POST["nuevoAbono"]));
				if ($nuevoAbono == $infoVenta["total"]) {
					$ulabono = (str_replace(",", "", $_POST["nuevoAbono"]));
					$nuevoAbono = 0;
					$metodo = "Completo";
				}
				...
				$fecha_abono = $fecha . ' ' . $hora;
				$ulabono = (str_replace(",", "", $_POST["nuevoAbono"]));
				$datos = array("id" => $_POST["idVentaAbo"], "metodo_pago" => $metodo, "id_vend_abono" => $_POST["idUsuarioAbo"], "abono" => $nuevoAbono, "fecha_abono" => $fecha_abono, "Ult_abono" => $ulabono);
				$respuesta = ModeloVentas::mdlActualizarDatosAbono("ventas", $datos);
				...
				if ($respuesta == "ok") {
					...
					echo '<script>
						swal({
							type:"error",
							title:"¡Ocurrió un error en la actualización de la información. Intentelo nueevamente!",
							showConfirmButton:true,
							confirmButtonText:"Cerrar"
						}).then(function(result){
							if(result.value){
				...
				echo '<script>
					swal({
						type:"error",
						title:"¡La información tiene caracteres especiales!",
						showConfirmButton:true,
						confirmButtonText:"Cerrar"
					}).then(function(result){
						if(result.value){
	...
	public static function reporteVenta()
	{
		...
		echo utf8_decode("<table border='0'> 
					...
					<td style='font-weight:bold; border:1px solid #eee;'>CLIENTE</td>
					<td style='font-weight:bold; border:1px solid #eee;'>EMP</td>
					<td style='font-weight:bold; border:1px solid #eee;'>VENDEDOR</td>
					<td style='font-weight:bold; border:1px solid #eee;'>V ABONO</td>
					<td style='font-weight:bold; border:1px solid #eee;'>FORMA DE PAGO</td>
					<td style='font-weight:bold; border:1px solid #eee;'>NETO</td>
					<td style='font-weight:bold; border:1px solid #eee;'>TOTAL</td>
					<td style='font-weight:bold; border:1px solid #eee;'>FECHA VENTA</td>
					<td style='font-weight:bold; border:1px solid #eee;'>ABONO</td>
					<td style='font-weight:bold; border:1px solid #eee;'>ULT ABONO</td>
		...
		foreach ($respuesta as $key => $value) {
			...
			echo utf8_decode("<tr>
			 			...
			 			<td style='border:1px solid #eee;'>" . $respuestaCliente["nombre"] . "</td> 
			 			<td style='border:1px solid #eee;'>" . (isset($respuestaUsuario_ab["empresa"]) ? $respuestaUsuario_ab["empresa"] : '') . "</td> 
			 			<td style='border:1px solid #eee;'>" . (isset($respuestaUsuario["nombre"]) ? $respuestaUsuario["nombre"] : '') . "</td> 
			 			<td style='border:1px solid #eee;'>" . (isset($respuestaUsuario_ab["nombre"]) ? $respuestaUsuario_ab["nombre"] : '') . "</td> 
			 			<td style='border:1px solid #eee;'>" . $value["metodo_pago"] . "</td> 
			 			<td style='border:1px solid #eee;'>$ " . numberFormat($value["neto"], 0) . "</td> 
			 			<td style='border:1px solid #eee;'>$ " . numberFormat($value["total"], 0) . "</td> 
			 			<td style='border:1px solid #eee;'>" . $value["fecha_abono"] . "</td> 
			 			<td style='border:1px solid #eee;'>$ " . numberFormat($value["abono"], 0) . "</td> 
			 			<td style='border:1px solid #eee;'>$ " . numberFormat($value["Ult_abono"], 0) . "</td> 
			 			<td style='border:1px solid #eee;'>" . $value["pago"] . "</td> 
			 			<td style='border:1px solid #eee;'>" . $value["medio_pago"] . "</td> 
		 			</tr>");
		}
